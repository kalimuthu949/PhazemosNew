...
export default class CommonService {
  ...
  /* Deva changes start */
  public SPGetChoices = async (
    customProperty: ICustomProperty,
    callBack: any
  ) => {
    await sp.web.lists
      .getByTitle(customProperty.listName)
      .fields.getByInternalNameOrTitle(customProperty.FieldName)
      .get()
      .then((res) => {
        callBack(res);
      });
  ...
  public SPDeleteItem = async (
    customProperty: ICustomProperty,
    callBack: any
  ) => {
    await sp.web.lists
      .getByTitle(customProperty.listName)
      .items.getById(customProperty.ID)
      .delete()
      .then((res) => {
        callBack(res);
      });
  ...
  public getPagedList(customProperty: ICustomProperty, callBack: any) {
    ...
    var skip =
      customProperty.currentPage * customProperty.recordsPerPage -
      customProperty.recordsPerPage;
    var web: IWeb = Web(this.webUrl);

    ////Get count of the list
    var getUrl =
      customProperty.context.pageContext.site.absoluteUrl +
      "/_api/web/lists/getbytitle('" +
      customProperty.listName +
      "')/ItemCount";
    ...
    customProperty.context.spHttpClient
      .get(getUrl, SPHttpClient.configurations.v1)
